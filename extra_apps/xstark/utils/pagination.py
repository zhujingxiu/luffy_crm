...
class Pagination(object):
    ...
    def pager_simple(self):
        page_list = []
        if self.current_page <= 1:
            prev = '<li><a href="#">上页</a></li>'
        else:
            self.query_params['page'] = self.current_page - 1
            prev = '<li><a href="%s?%s">上页</a></li>' % (self.base_url,self.query_params.urlencode())
        page_list.append(prev)
        if self.current_page >= self.pager_count:
            nex = '<li><a href="#">下页</a></li>'
        else:
            self.query_params['page'] = self.current_page + 1
            nex = '<li><a href="%s?%s">下页</a></li>' % (self.base_url, self.query_params.urlencode(),)
    ...
    def page_html(self):
        ...
        page_list.append(nex)
        if self.pager_count > 1:
            page_list.append('<li class="disabled"><a>%s条 / %s页</a></li>'% (self.all_count, self.pager_count))
        page_str = "".join(page_list)
        return page_str
