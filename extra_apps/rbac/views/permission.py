...
def menu_list(request):
    ...
    # 找到可以成为菜单的权限的所有子权限
    node_permission_list = models.Permission.objects.filter(parent__in=permissions).order_by('-id').values('id',
                                                                                                        'title',
                                                                                                        'url',
                                                                                                        'name',
                                                                                                        'parent')
...
def distribute_permissions(request):
    ...
    for per in node_permission:
        parent = per['parent_id']
        if not parent:
            menu_dict[None]['children'].append(per)
            continue
        root_permission_dict[parent]['children'].append(per)
